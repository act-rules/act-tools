...
describe('findProcedureSet', () => {
  ...
  const partialProcedure1: ActProcedureMapping = {
    ...procedureDefaults,
    testResults: [correctPass, falseNegativeFail]
  }
  ...
  const partialAutomaticProcedure: ActProcedureMapping = {
    ...procedureDefaults,
    consistentRequirements: false,
    testResults: [{
      ...correctPass,
  ...
  it('reports partial over minimal or inconsistent', () => {
    ...
    expect(procedureSet.procedures).toEqual([partialProcedure1, partialProcedure2]);
  });
  ...
  it('sorts results from most automated to least', () => {
    const procedureSet = findProcedureSet([
      partialProcedure2,
      partialAutomaticProcedure,
      partialProcedure1
    ]);
    expect(procedureSet.procedures.indexOf(partialAutomaticProcedure)).toBe(0);
    expect(procedureSet.procedures.indexOf(partialProcedure2)).toBe(1);
    expect(procedureSet.procedures.indexOf(partialProcedure1)).toBe(2);
  });
